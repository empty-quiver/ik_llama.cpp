...
private:
    ...
    static ggml_tensor * build_qkv(ggml_context * ctx0, ggml_tensor * state_storage, ggml_tensor * ssm_conv1d,
            ...
            // Fix 1a: when non-null, emit ggml_cpy(s_l[il] -> spec_ckpt_shadow)
            // at the start of this layer's build (before any state mutation),
            // so the SSM-state checkpoint rides the verify cgraph and avoids
            // a pre-decode synchronous tensor_get loop.
