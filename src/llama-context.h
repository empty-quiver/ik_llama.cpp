...
struct llama_kv_cache {
    ...
    // PR3: legacy gpu_checkpoint / shadow / per-step machinery removed. Hybrid
    // (qnext) speculative decoding now uses metadata-only seq_cp paired with an
    // eager D2D row copy of the recurrent state (see llama_kv_cache_qnext_copy_row
    // in src/llama.cpp). The public llama_spec_ckpt_* C API is preserved as
    // deprecated no-op stubs for one release.
...
struct llama_context {
    ...
    struct ggml_tensor * inp_s_copy_qnext; // I32 [1, n_batch] (gather row index into qnext state_storage; was inp_s_seq_qnext)
